...
- 上传接口到质衡：IDE 装 [Qualitest Helper](https://plugins.jetbrains.com/plugin/34434-qualitest-helper)，扫 Controller（`@api.group`）上传到 `http://localhost:8800`；上传后按上一节补齐双端鉴权配置再造流。步骤见插件仓 [使用方式](https://github.com/qualitest-hq/qualitest-intellij-plugin#使用方式)
